...
module Data.BitVector.Internal
    ( BitVector (..)
    , dimension
    , fromBits
    , fromNumber
...
instance Num BitVector where
    ...
    fromInteger intValue =
        let !intBits = I# (integerLog2# intValue)
        in  fromNumber (toEnum intBits) intValue
...
{-# INLINE modular #-}
modular :: (Integer -> Integer -> Integer) -> BitVector -> BitVector -> BitVector
modular op (BV w1 lhs) (BV w2 rhs) =
          let dim = max w1 w2
              lim = 1 `shiftL` fromEnum dim
              res = (toInteger lhs `op` toInteger rhs) `mod` lim
          in  BV dim $ fromInteger res
